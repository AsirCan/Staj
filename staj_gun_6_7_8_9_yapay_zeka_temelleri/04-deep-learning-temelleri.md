...
"Sinir ağı" adı biyolojik nöronlardan geliyor ama modern sinir ağları biyolojik beynin bir modeli değil. Sadece isim benzerliği var. Biraz ilham almış, o kadar. Bu benzetmeyi fazla ciddiye almamak gerekiyor.
...
- **Dense / Linear / Fully Connected**: yukarıda anlatılan tür. Her giriş her nörona bağlı.
...
- **Attention / Transformer katmanı**: modern NLP'nin temeli. Ayrı bir dosyada detaylı ele alınıyor.
...
Şimdilik ilk tür (Dense) yeterli, geri kalanı sırayla ele alınıyor.
...
PyTorch'un `autograd` diye bir motoru var. Forward pass yazılırken, o arka planda "hesap grafiği" (computational graph) tutuyor. `.backward()` çağrılınca bu grafik boyunca gradyanları hesaplıyor. Bu, 06'daki PyTorch Blitz'de kod üstünde görülüyor.
...
Çıktı olasılık dağılımı üretmek için. `01-olasilik-istatistik.md`'de anlatıldı.
...
`01-olasilik-istatistik.md`'de bias-variance olarak anlatılmıştı. Burada pratik gözlem:
...
PyTorch'ta bu döngü ~15 satırlık gerçek koda dönüşüyor. `06-pytorch-60-min-blitz/` klasöründe görülüyor.
...
Bağlantısı: yukarıda anlatılan her şey (autograd, backprop, katmanlar, optimizer'lar) framework'lerin sağladığı hazır parçalar. Bir sonraki bölüm hangi framework'ün hangi işi ne kadar kolaylaştırdığına bakıyor. Ardından `06-` klasöründe bunlar PyTorch ile elle koda dökülüyor.
