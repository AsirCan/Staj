...
Bunu çalıştırınca yaşın ortalamasını verir. `df`'yi tekrar okumadım, hafızada duruyor.
...
Modern IDE'lerin çoğunda `.ipynb` desteği var. Sağda değişken izleyici, aşağıda çıktı — Jupyter deneyimini IDE içinde veriyor. Ben genelde bunu tercih ediyorum çünkü aynı ekranda `.py` dosyalarımla bakabiliyorum.

### 2.5 "Interactive Window" (VS Code'da `# %%`)

Bir de düz `.py` dosyasında `# %%` yorumuyla hücre işaretlemek var. Böylece dosya `.py` kalıyor (git'e temiz düşüyor) ama VS Code hücre hücre çalıştırabiliyor. Küçük deneylerde bu yolu seviyorum.
...
Diyelim ki bir CSV üzerinde EDA yapıyorum. Sırayla:
...
Her hücre 1 iş yapıyor, çıktısını görüyorum, sonrakine karar veriyorum. Bir hata çıkarsa sadece o hücreyi düzeltiyorum.

**Aynı işi `.py` betiğinde yapsam** her seferinde tüm veriyi baştan okumam, tüm hesabı baştan yapmam gerekirdi. Küçük veride problem değil ama 5 GB CSV'de zaman öldürücü.
...
Küçük kişisel projelerimde bunlarla uğraşmıyorum ama takım projesinde şart.

---

## 8. AI / ML dünyasındaki yeri

Notebook'un özellikle ML projelerinde neden bu kadar sık kullanıldığını şöyle özetliyorum:
...
Yani şöyle diyebilirim: **notebook, AI dünyasının "laboratuvarı"**. Deney yapılan yer.
...
Bağlantısı basit: ML temellerini öğrenirken göreceğim eğitim döngüsünü (veri → model → tahmin → loss → güncelleme) küçük ölçekte kendi gözümle görebilmek için notebook en uygun yer. Yani "teori orada, deney burada" ilişkisi var.
